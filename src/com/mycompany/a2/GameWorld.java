...
public class GameWorld extends Observable {
	...
	/**
	 * Add the object that is passed as a parameter to the game object collection.
	 * @param obj, which is a game object.
	 */
	...
	/**
	 * @return integer gameClock value.
	 */
	...
	public void brake() {
	    ...
	    while (iterator.hasNext()) {
	        GameObject obj = iterator.getNext();
	        if (obj instanceof Ant) {
	        	System.out.println("Braking, so decreasing speed by 1.");
	            Ant aObj = (Ant) obj;
	            if (aObj.getSpeed() > 0) {
	                aObj.setSpeed(aObj.getSpeed() - 1);
	                this.setChanged();
	    ...
	    do {
	        ...
	        if (text != null && !text.isEmpty()) {
	            try {
	                ...
	                Dialog.show("Error", "Please enter a valid numeric value", "OK", null);
	            }
	        }
	        else if (flagNum == -1 || flagNum < 1 || flagNum > lastFlag) {
...
*/
	...
	public void collidedFoodStation() {
	    ...
	    while (iterator.hasNext()) {
	        ...
	        } else if (obj instanceof FoodStation && !(randomFoodStation == foodStationCounter)) { 
	        	// instance of FoodStation but NOT the right food station that was randomly picked
	...
	public void collidedSpider() {
	    ...
	    while (iterator.hasNext()) {
	        ...
	        if (obj instanceof Ant) {
	            ...
	            //System.out.println(aObj.getHealthLevel());
	            //System.out.println(aObj.getMaxHealthLevel());
	            
	            aObj.setSpeed((int)((double)aObj.getHealthLevel()/(double)aObj.getMaxHealthLevel() * aObj.getSpeed()));
	            if (aObj.getFoodLevel() == 0 || aObj.getHealthLevel() == 0) {
	            	System.out.println("Speed:" + aObj.getSpeed());
	                ...
	                if (this.antLivesLeft == 0) {
	                	...
	                	int red = ColorUtil.red(aObj.getColor());
	                	int green = ColorUtil.green(aObj.getColor());
	                	int blue = ColorUtil.blue(aObj.getColor());

	                	// Lighten the red component
	                	red = Math.min(255, red + 10);

	                	int newColor = ColorUtil.rgb(red, green, blue);
	                	aObj.setColor(newColor);
	                    // iterator.remove();
	                    System.gc();
	...
	/**
	 * Exit the application gracefully using exitApplication().
	 */
	public void exit() {
	...
	/**
	 * Sound is a boolean that will be used in A3 to incorporate sound.
	 * Here we allow the user to "switch" the sound on and off.
	 */
	...
	/**
	 * @return string denoting if the sound is on or off.
	 */
	...
	/**
	 * Set the width the map.
	 * @param mapWidth, and integer value.
	 */
	 ...
	 * @param mapHeight, an integer value.
	 */
